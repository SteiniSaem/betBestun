<script>
    let { option1, option2, fontSize = 16, value = 1 } = $props()

    let selectedOption = $state(value == 1 ? option1 : option2)
    ...
    function handleClick(event){
        ...
        selectedOption = state === 'true' ? option1 : option2

        value = selectedOption == option1 ? 1 : 2
    }
...
<div class="s s--slider flex" style="font-size:{fontSize}px">
    <p>{option1}</p>
    <button
        role="switch"
